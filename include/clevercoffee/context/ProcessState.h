...
public:
    ...
    void setSetpoint(double setpoint) noexcept { setpoint_ = setpoint; }

    // Steam setpoint
    double steamSetpoint() const noexcept { return steamSetpointValue_; }
    void setSteamSetpoint(double setpoint) noexcept { steamSetpointValue_ = setpoint; }

    // PID Output
    double pidOutput() const noexcept { return pidOutput_; }
    void setPidOutput(double output) noexcept { pidOutput_ = output; }

    // PID Enabled
    bool pidEnabled() const noexcept { return pidEnabled_; }
    void setPidEnabled(bool enabled) noexcept { pidEnabled_ = enabled; }

    // Brew Time
    double currentBrewTime() const noexcept { return currBrewTime_; }
    void setCurrentBrewTime(double time) noexcept { currBrewTime_ = time; }

    // Starting Time
    long startingTime() const noexcept { return startingTime_; }
    void setStartingTime(long time) noexcept { startingTime_ = time; }

    // Target Brew Time
    double totalTargetBrewTime() const noexcept { return totalTargetBrewTime_; }
    void setTotalTargetBrewTime(double time) noexcept { totalTargetBrewTime_ = time; }

    // Brew PID Disabled
    bool brewPidDisabled() const noexcept { return brewPidDisabled_; }
    void setBrewPidDisabled(bool disabled) noexcept { brewPidDisabled_ = disabled; }

    // Previous Input (for PID derivative)
    double previousInput() const noexcept { return previousInput_; }
    void setPreviousInput(double input) noexcept { previousInput_ = input; }

    // PID Parameters - Aggressive (Brew Detection)
    double pidAggKi() const noexcept { return aggbKi_; }
    void setPidAggKi(double value) noexcept { aggbKi_ = value; }

    double pidAggKd() const noexcept { return aggbKd_; }
    void setPidAggKd(double value) noexcept { aggbKd_ = value; }

    // PID Parameters - Normal
    double pidKi() const noexcept { return aggKi_; }
    void setPidKi(double value) noexcept { aggKi_ = value; }

    double pidKd() const noexcept { return aggKd_; }
    void setPidKd(double value) noexcept { aggKd_ = value; }

    // PID Window Size
    int windowSize() const noexcept { return windowSize_; }
    void setWindowSize(int size) noexcept { windowSize_ = size; }

    // Pointer accessors for PID controller (backward compatibility)
    double* temperaturePtr() noexcept { return &temperature_; }
    double* pidOutputPtr() noexcept { return &pidOutput_; }
    double* setpointPtr() noexcept { return &setpoint_; }
