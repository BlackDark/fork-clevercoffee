...
constexpr bool isInputPin(int pin) noexcept {
    return pin == PIN_POWERSWITCH || pin == PIN_BREWSWITCH || pin == PIN_STEAMSWITCH || pin == PIN_WATERSWITCH ||
           pin == PIN_ROTARY_DT || pin == PIN_ROTARY_CLK || pin == PIN_ROTARY_SW || pin == PIN_TEMPSENSOR ||
           pin == PIN_WATERTANKSENSOR || pin == PIN_HXDAT || pin == PIN_HXDAT2 || pin == PIN_HXCLK || pin == PIN_ZC;
}

constexpr bool isOutputPin(int pin) noexcept {
    return pin == PIN_VALVE || pin == PIN_PUMP || pin == PIN_HEATER || pin == PIN_STATUSLED || pin == PIN_BREWLED ||
           pin == PIN_STEAMLED;
}
