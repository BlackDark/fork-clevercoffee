...
export function areRequiredParametersMet(
	...
	for (const [paramName, expectedValue] of Object.entries(parameter.requiredParameters)) {
...
export function getMissingRequiredParametersMessage(
	...
	for (const [paramName, expectedValue] of Object.entries(parameter.requiredParameters)) {
		...
		if (actualValue !== expectedValue) {
			...
			const expectedLabel = expectedValue === 1 ? 'enabled' : `set to ${expectedValue}`;
 ...
 */
export function groupParametersByPrefix(parameters: Parameter[]): Record<string, Parameter[]> {
 ...
 */
export function groupParametersBySection(parameters: Parameter[]): Record<string, Parameter[]> {
	const visibleParameters = parameters.filter(() => true); // Show all parameters
	...
	for (const param of visibleParameters) {
		...
		if (groupLabel) {
			...
			const prefix = param.name.split('.')[0];
			const fallbackLabel = `${prefix.charAt(0).toUpperCase()}${prefix.slice(1)} Parameters`;
