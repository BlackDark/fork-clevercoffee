// Complete parameter definitions with default values and conditions
// This ensures all parameters are available in the UI even if not sent by the server

export interface ParameterCondition {
  showWhen?: Record<string, any>;
  hideWhen?: Record<string, any>;
}

export interface CompleteParameter {
  type: number;
  name: string;
  displayName: string;
  section: number;
  position: number;
  hasHelpText: boolean;
  show: boolean;
  value: any;
  min: number;
  max: number;
  options?: Array<{ value: number; label: string }>;
  conditions?: ParameterCondition;
}

// All parameters with their default values and conditions
export const allParameters: CompleteParameter[] = [
  // PID Parameters (Section 0)
  {
    type: 1,
    name: "pid.enabled",
    displayName: "Enable PID Controller",
    section: 0,
    position: 101,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "pid.use_ponm",
    displayName: "Enable PonM",
    section: 0,
    position: 102,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 2,
    name: "pid.ema_factor",
    displayName: "PID EMA Factor",
    section: 0,
    position: 111,
    hasHelpText: true,
    show: true,
    value: 0.6,
    min: 0,
    max: 1,
  },
  {
    type: 2,
    name: "pid.regular.kp",
    displayName: "PID Kp",
    section: 0,
    position: 112,
    hasHelpText: true,
    show: true,
    value: 62,
    min: 0,
    max: 200,
  },
  {
    type: 2,
    name: "pid.regular.tn",
    displayName: "PID Tn (=Kp/Ki)",
    section: 0,
    position: 113,
    hasHelpText: true,
    show: true,
    value: 52,
    min: 0,
    max: 200,
  },
  {
    type: 2,
    name: "pid.regular.tv",
    displayName: "PID Tv (=Kd/Kp)",
    section: 0,
    position: 114,
    hasHelpText: true,
    show: true,
    value: 11.5,
    min: 0,
    max: 200,
  },
  {
    type: 2,
    name: "pid.regular.i_max",
    displayName: "PID Integrator Max",
    section: 0,
    position: 115,
    hasHelpText: true,
    show: true,
    value: 55,
    min: 0,
    max: 100,
  },
  {
    type: 2,
    name: "pid.steam.kp",
    displayName: "Steam Kp",
    section: 0,
    position: 116,
    hasHelpText: true,
    show: true,
    value: 150,
    min: 0,
    max: 500,
  },

  // Temperature Control (Section 1)
  {
    type: 2,
    name: "TEMP",
    displayName: "Temperature",
    section: 1,
    position: 200,
    hasHelpText: false,
    show: false, // Read-only, usually hidden
    value: 0,
    min: 0,
    max: 200,
  },
  {
    type: 2,
    name: "brew.setpoint",
    displayName: "Setpoint (°C)",
    section: 1,
    position: 201,
    hasHelpText: true,
    show: true,
    value: 95,
    min: 20,
    max: 110,
  },
  {
    type: 2,
    name: "brew.temp_offset",
    displayName: "Offset (°C)",
    section: 1,
    position: 202,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 20,
  },
  {
    type: 2,
    name: "steam.setpoint",
    displayName: "Steam Setpoint (°C)",
    section: 1,
    position: 203,
    hasHelpText: true,
    show: true,
    value: 120,
    min: 100,
    max: 140,
  },

  // Brew PID Parameters (Section 2)
  {
    type: 1,
    name: "pid.bd.enabled",
    displayName: "Enable Brew PID",
    section: 2,
    position: 701,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "brew.pid_delay",
    displayName: "Brew PID Delay (s)",
    section: 2,
    position: 711,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 60,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "pid.bd.kp",
    displayName: "BD Kp",
    section: 2,
    position: 712,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 200,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "pid.bd.tn",
    displayName: "BD Tn (=Kp/Ki)",
    section: 2,
    position: 713,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 200,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "pid.bd.tv",
    displayName: "BD Tv (=Kd/Kp)",
    section: 2,
    position: 714,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 200,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },

  // Brew Control (Section 3) - THE MISSING PARAMETERS!
  {
    type: 5,
    name: "brew.mode",
    displayName: "Brew Mode",
    section: 3,
    position: 301,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Manual" },
      { value: 1, label: "Automatic" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "brew.by_time", // HERE IT IS!
    displayName: "Brew by Time",
    section: 3,
    position: 311,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
        "brew.mode": 1,
      },
    },
  },
  {
    type: 2,
    name: "brew.target_time",
    displayName: "Target Brew Time (s)",
    section: 3,
    position: 312,
    hasHelpText: true,
    show: true,
    value: 25,
    min: 1,
    max: 300,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
        "brew.mode": 1,
      },
    },
  },
  {
    type: 1,
    name: "brew.by_weight",
    displayName: "Brew by Weight",
    section: 3,
    position: 321,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
        "brew.mode": 1,
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "brew.target_weight",
    displayName: "Target Brew Weight (g)",
    section: 3,
    position: 322,
    hasHelpText: true,
    show: true,
    value: 36,
    min: 1,
    max: 500,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
        "brew.mode": 1,
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "brew.pre_infusion.enabled",
    displayName: "Pre-Infusion",
    section: 3,
    position: 331,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "brew.pre_infusion.time",
    displayName: "Pre-infusion Time (s)",
    section: 3,
    position: 332,
    hasHelpText: true,
    show: true,
    value: 2,
    min: 0,
    max: 30,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "brew.pre_infusion.pause",
    displayName: "Pre-infusion Pause Time (s)",
    section: 3,
    position: 333,
    hasHelpText: true,
    show: true,
    value: 8,
    min: 0,
    max: 30,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },

  // Scale Parameters (Section 4)
  {
    type: 3,
    name: "hardware.sensors.scale.known_weight",
    displayName: "Known Calibration Weight",
    section: 4,
    position: 601,
    hasHelpText: true,
    show: true,
    value: 267,
    min: 1,
    max: 2000,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 3,
    name: "hardware.sensors.scale.calibration",
    displayName: "Calibration Factor Scale 1",
    section: 4,
    position: 602,
    hasHelpText: true,
    show: true,
    value: 1,
    min: -100000,
    max: 100000,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 3,
    name: "hardware.sensors.scale.calibration2",
    displayName: "Calibration Factor Scale 2",
    section: 4,
    position: 603,
    hasHelpText: true,
    show: true,
    value: 1,
    min: -100000,
    max: 100000,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
        "hardware.sensors.scale.type": 0,
      },
    },
  },

  // Display Settings (Section 5)
  {
    type: 5,
    name: "display.template",
    displayName: "Display Template",
    section: 5,
    position: 901,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Standard" },
      { value: 1, label: "Minimal" },
      { value: 2, label: "Temp only" },
      { value: 3, label: "Scale" },
      { value: 4, label: "Upright" },
    ],
    min: 0,
    max: 4,
  },
  {
    type: 1,
    name: "display.inverted",
    displayName: "Invert Display",
    section: 5,
    position: 902,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "display.language",
    displayName: "Display Language",
    section: 5,
    position: 903,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Deutsch" },
      { value: 1, label: "English" },
      { value: 2, label: "Español" },
    ],
    min: 0,
    max: 2,
  },
  {
    type: 1,
    name: "display.fullscreen_brew_timer",
    displayName: "Enable Fullscreen Brew Timer",
    section: 5,
    position: 904,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "display.fullscreen_manual_flush_timer",
    displayName: "Enable Fullscreen Manual Flush Timer",
    section: 5,
    position: 905,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "display.fullscreen_hot_water_timer",
    displayName: "Enable Fullscreen Hot Water Timer",
    section: 5,
    position: 906,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 2,
    name: "display.post_brew_timer_duration",
    displayName: "Post Brew Timer Duration (s)",
    section: 5,
    position: 907,
    hasHelpText: true,
    show: true,
    value: 3,
    min: 0,
    max: 60,
  },
  {
    type: 1,
    name: "display.heating_logo",
    displayName: "Enable Heating Logo",
    section: 5,
    position: 908,
    hasHelpText: true,
    show: true,
    value: 1,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "display.pid_off_logo",
    displayName: "Enable 'PID Disabled' Logo",
    section: 5,
    position: 909,
    hasHelpText: true,
    show: true,
    value: 1,
    min: 0,
    max: 1,
  },

  // Maintenance (Section 6)
  {
    type: 0,
    name: "backflush.cycles",
    displayName: "Backflush Cycles",
    section: 6,
    position: 401,
    hasHelpText: true,
    show: true,
    value: 5,
    min: 1,
    max: 20,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "backflush.fill_time",
    displayName: "Backflush Fill Time (s)",
    section: 6,
    position: 402,
    hasHelpText: true,
    show: true,
    value: 5,
    min: 1,
    max: 30,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "backflush.flush_time",
    displayName: "Backflush Flush Time (s)",
    section: 6,
    position: 403,
    hasHelpText: true,
    show: true,
    value: 10,
    min: 1,
    max: 30,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },

  // Power Settings (Section 7)
  {
    type: 1,
    name: "standby.enabled",
    displayName: "Enable Standby Timer",
    section: 7,
    position: 801,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 2,
    name: "standby.time",
    displayName: "Standby Time",
    section: 7,
    position: 802,
    hasHelpText: true,
    show: true,
    value: 35,
    min: 1,
    max: 120,
  },

  // MQTT Settings (Section 8)
  {
    type: 1,
    name: "mqtt.enabled",
    displayName: "MQTT enabled",
    section: 8,
    position: 1001,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 4,
    name: "mqtt.broker",
    displayName: "Hostname",
    section: 8,
    position: 1011,
    hasHelpText: true,
    show: true,
    value: "",
    min: 0,
    max: 64,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
      },
    },
  },
  {
    type: 0,
    name: "mqtt.port",
    displayName: "Port",
    section: 8,
    position: 1012,
    hasHelpText: true,
    show: true,
    value: 1883,
    min: 1,
    max: 65535,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
      },
    },
  },
  {
    type: 4,
    name: "mqtt.username",
    displayName: "Username",
    section: 8,
    position: 1013,
    hasHelpText: true,
    show: true,
    value: "rancilio",
    min: 0,
    max: 32,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
      },
    },
  },
  {
    type: 4,
    name: "mqtt.password",
    displayName: "Password",
    section: 8,
    position: 1014,
    hasHelpText: true,
    show: true,
    value: "silvia",
    min: 0,
    max: 64,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
      },
    },
  },
  {
    type: 4,
    name: "mqtt.topic",
    displayName: "Topic Prefix",
    section: 8,
    position: 1015,
    hasHelpText: true,
    show: true,
    value: "custom/kitchen/",
    min: 0,
    max: 48,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "mqtt.hassio.enabled",
    displayName: "Hass.io enabled",
    section: 8,
    position: 1021,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
      },
    },
  },
  {
    type: 4,
    name: "mqtt.hassio.prefix",
    displayName: "Hass.io Prefix",
    section: 8,
    position: 1022,
    hasHelpText: true,
    show: true,
    value: "homeassistant",
    min: 0,
    max: 24,
    conditions: {
      showWhen: {
        "mqtt.enabled": 1,
        "mqtt.hassio.enabled": 1,
      },
    },
  },

  // System Settings (Section 9)
  {
    type: 4,
    name: "system.hostname",
    displayName: "Hostname",
    section: 9,
    position: 1101,
    hasHelpText: true,
    show: true,
    value: "silvia",
    min: 0,
    max: 64,
  },
  {
    type: 4,
    name: "system.ota_password",
    displayName: "OTA Password",
    section: 9,
    position: 1102,
    hasHelpText: true,
    show: true,
    value: "otapass",
    min: 0,
    max: 64,
  },
  {
    type: 5,
    name: "system.log_level",
    displayName: "Log Level",
    section: 9,
    position: 1103,
    hasHelpText: true,
    show: true,
    value: 2,
    options: [
      { value: 0, label: "TRACE" },
      { value: 1, label: "DEBUG" },
      { value: 2, label: "INFO" },
      { value: 3, label: "WARNING" },
      { value: 4, label: "ERROR" },
      { value: 5, label: "FATAL" },
      { value: 6, label: "SILENT" },
    ],
    min: 0,
    max: 6,
  },
  {
    type: 1,
    name: "system.auth.enabled",
    displayName: "Enable Website Authentication",
    section: 9,
    position: 1201,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 4,
    name: "system.auth.username",
    displayName: "Website Username",
    section: 9,
    position: 1202,
    hasHelpText: true,
    show: true,
    value: "admin",
    min: 0,
    max: 32,
    conditions: {
      showWhen: {
        "system.auth.enabled": 1,
      },
    },
  },
  {
    type: 4,
    name: "system.auth.password",
    displayName: "Website Password",
    section: 9,
    position: 1203,
    hasHelpText: true,
    show: true,
    value: "admin",
    min: 0,
    max: 64,
    conditions: {
      showWhen: {
        "system.auth.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "system.timing_debug.enabled",
    displayName: "Loop timing in console",
    section: 9,
    position: 1301,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "system.log_level": 1,
      },
    },
  },
  {
    type: 1,
    name: "system.showdisplay.enabled",
    displayName: "Activate display recording in debug logs",
    section: 9,
    position: 1303,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "system.log_level": 1,
      },
    },
  },

  // Runtime Controls (Section 10)
  {
    type: 1,
    name: "STEAM_MODE",
    displayName: "Steam Mode",
    section: 10,
    position: 503,
    hasHelpText: false,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "BACKFLUSH_ON",
    displayName: "Backflush",
    section: 10,
    position: 504,
    hasHelpText: false,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "TARE_ON",
    displayName: "Tare",
    section: 10,
    position: 501,
    hasHelpText: false,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "CALIBRATION_ON",
    displayName: "Calibration",
    section: 10,
    position: 502,
    hasHelpText: false,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 4,
    name: "VERSION",
    displayName: "Version",
    section: 10,
    position: 7,
    hasHelpText: false,
    show: false,
    value: "",
    min: 0,
    max: 64,
  },

  // Hardware - OLED Display (Section 11)
  {
    type: 1,
    name: "hardware.oled.enabled",
    displayName: "Enable OLED Display",
    section: 11,
    position: 2001,
    hasHelpText: true,
    show: true,
    value: 1,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.oled.type",
    displayName: "OLED Type",
    section: 11,
    position: 2002,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "SH1106" },
      { value: 1, label: "SSD1306" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.oled.enabled": 1,
      },
    },
  },
  {
    type: 5,
    name: "hardware.oled.address",
    displayName: "I2C Address",
    section: 11,
    position: 2003,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "0x3C" },
      { value: 1, label: "0x3D" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.oled.enabled": 1,
      },
    },
  },

  // Hardware - Relays (Section 12)
  {
    type: 5,
    name: "hardware.relays.heater.trigger_type",
    displayName: "Heater Relay Trigger Type",
    section: 12,
    position: 2101,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Low Trigger" },
      { value: 1, label: "High Trigger" },
    ],
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.relays.valve.trigger_type",
    displayName: "Valve Relay Trigger Type",
    section: 12,
    position: 2102,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Low Trigger" },
      { value: 1, label: "High Trigger" },
    ],
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.relays.pump.trigger_type",
    displayName: "Pump Relay Trigger Type",
    section: 12,
    position: 2103,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Low Trigger" },
      { value: 1, label: "High Trigger" },
    ],
    min: 0,
    max: 1,
  },

  // Hardware - Switches (Section 13)
  {
    type: 1,
    name: "hardware.switches.brew.enabled",
    displayName: "Enable Brew Switch",
    section: 13,
    position: 2201,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.switches.brew.type",
    displayName: "Brew Switch Type",
    section: 13,
    position: 2202,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Momentary" },
      { value: 1, label: "Toggle" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 5,
    name: "hardware.switches.brew.mode",
    displayName: "Brew Switch Mode",
    section: 13,
    position: 2203,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Normally Open" },
      { value: 1, label: "Normally Closed" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.brew.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "hardware.switches.steam.enabled",
    displayName: "Enable Steam Switch",
    section: 13,
    position: 2211,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.switches.steam.type",
    displayName: "Steam Switch Type",
    section: 13,
    position: 2212,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Momentary" },
      { value: 1, label: "Toggle" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.steam.enabled": 1,
      },
    },
  },
  {
    type: 5,
    name: "hardware.switches.steam.mode",
    displayName: "Steam Switch Mode",
    section: 13,
    position: 2213,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Normally Open" },
      { value: 1, label: "Normally Closed" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.steam.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "hardware.switches.power.enabled",
    displayName: "Enable Power Switch",
    section: 13,
    position: 2221,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.switches.power.type",
    displayName: "Power Switch Type",
    section: 13,
    position: 2222,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Momentary" },
      { value: 1, label: "Toggle" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.power.enabled": 1,
      },
    },
  },
  {
    type: 5,
    name: "hardware.switches.power.mode",
    displayName: "Power Switch Mode",
    section: 13,
    position: 2223,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Normally Open" },
      { value: 1, label: "Normally Closed" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.power.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "hardware.switches.hot_water.enabled",
    displayName: "Enable Water Switch",
    section: 13,
    position: 2231,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.switches.hot_water.type",
    displayName: "Water Switch Type",
    section: 13,
    position: 2232,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Momentary" },
      { value: 1, label: "Toggle" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.hot_water.enabled": 1,
      },
    },
  },
  {
    type: 5,
    name: "hardware.switches.hot_water.mode",
    displayName: "Water Switch Mode",
    section: 13,
    position: 2233,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "Normally Open" },
      { value: 1, label: "Normally Closed" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.switches.hot_water.enabled": 1,
      },
    },
  },

  // Hardware - LEDs (Section 14)
  {
    type: 1,
    name: "hardware.leds.status.enabled",
    displayName: "Enable Status LED",
    section: 14,
    position: 2301,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "hardware.leds.status.inverted",
    displayName: "Invert Status LED",
    section: 14,
    position: 2302,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.leds.status.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "hardware.leds.brew.enabled",
    displayName: "Enable Brew LED",
    section: 14,
    position: 2311,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "hardware.leds.brew.inverted",
    displayName: "Invert Brew LED",
    section: 14,
    position: 2312,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.leds.brew.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "hardware.leds.steam.enabled",
    displayName: "Enable Steam LED",
    section: 14,
    position: 2321,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "hardware.leds.steam.inverted",
    displayName: "Invert Steam LED",
    section: 14,
    position: 2322,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.leds.steam.enabled": 1,
      },
    },
  },

  // Hardware - Sensors (Section 15)
  {
    type: 5,
    name: "hardware.sensors.temperature.type",
    displayName: "Temperature Sensor Type",
    section: 15,
    position: 2401,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "TSIC306" },
      { value: 1, label: "Dallas DS18B20" },
    ],
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "hardware.sensors.pressure.enabled",
    displayName: "Enable Pressure Sensor",
    section: 15,
    position: 2411,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 1,
    name: "hardware.sensors.watertank.enabled",
    displayName: "Enable Water Tank Sensor",
    section: 15,
    position: 2421,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.sensors.watertank.mode",
    displayName: "Water Tank Sensor Mode",
    section: 15,
    position: 2422,
    hasHelpText: true,
    show: true,
    value: 1,
    options: [
      { value: 0, label: "Normally Open" },
      { value: 1, label: "Normally Closed" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.sensors.watertank.enabled": 1,
      },
    },
  },
  {
    type: 1,
    name: "hardware.sensors.scale.enabled",
    displayName: "Enable Scale",
    section: 15,
    position: 2431,
    hasHelpText: true,
    show: true,
    value: 0,
    min: 0,
    max: 1,
  },
  {
    type: 5,
    name: "hardware.sensors.scale.type",
    displayName: "Scale Setup Type",
    section: 15,
    position: 2432,
    hasHelpText: true,
    show: true,
    value: 0,
    options: [
      { value: 0, label: "2 load cells" },
      { value: 1, label: "1 load cell" },
    ],
    min: 0,
    max: 1,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 0,
    name: "hardware.sensors.scale.samples",
    displayName: "Scale Samples",
    section: 15,
    position: 2433,
    hasHelpText: true,
    show: true,
    value: 2,
    min: 1,
    max: 20,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "hardware.sensors.scale.calibration",
    displayName: "Scale Calibration Factor",
    section: 15,
    position: 2434,
    hasHelpText: true,
    show: true,
    value: 1,
    min: -10000,
    max: 10000,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
  {
    type: 2,
    name: "hardware.sensors.scale.calibration2",
    displayName: "Scale Calibration Factor 2",
    section: 15,
    position: 2435,
    hasHelpText: true,
    show: true,
    value: 1,
    min: -10000,
    max: 10000,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
        "hardware.sensors.scale.type": 0,
      },
    },
  },
  {
    type: 2,
    name: "hardware.sensors.scale.known_weight",
    displayName: "Known Calibration Weight",
    section: 15,
    position: 2436,
    hasHelpText: true,
    show: true,
    value: 267,
    min: 1,
    max: 5000,
    conditions: {
      showWhen: {
        "hardware.sensors.scale.enabled": 1,
      },
    },
  },
];

/**
 * Evaluates whether a parameter should be shown based on its conditions
 */
export function shouldShowParameter(
  parameter: CompleteParameter,
  allParameters: CompleteParameter[]
): boolean {
  if (!parameter.conditions) {
    return parameter.show;
  }

  const parameterMap = new Map(allParameters.map((p) => [p.name, p.value]));

  // Check showWhen conditions
  if (parameter.conditions.showWhen) {
    for (const [paramName, expectedValue] of Object.entries(
      parameter.conditions.showWhen
    )) {
      const actualValue = parameterMap.get(paramName);
      if (actualValue !== expectedValue) {
        return false;
      }
    }
  }

  // Check hideWhen conditions
  if (parameter.conditions.hideWhen) {
    for (const [paramName, expectedValue] of Object.entries(
      parameter.conditions.hideWhen
    )) {
      const actualValue = parameterMap.get(paramName);
      if (actualValue === expectedValue) {
        return false;
      }
    }
  }

  return parameter.show;
}

/**
 * Merges server parameters with complete parameter definitions
 */
export function mergeWithServerParameters(
  serverParameters: any[],
  completeParameters: CompleteParameter[] = allParameters
): CompleteParameter[] {
  const serverParamMap = new Map(serverParameters.map((p) => [p.name, p]));

  return completeParameters.map((completeParam) => {
    const serverParam = serverParamMap.get(completeParam.name);
    if (serverParam) {
      // Use server values but keep our complete definition structure
      return {
        ...completeParam,
        value: serverParam.value,
        // Optionally override other properties from server if needed
        show:
          serverParam.show !== undefined
            ? serverParam.show
            : completeParam.show,
      };
    }
    return completeParam;
  });
}
