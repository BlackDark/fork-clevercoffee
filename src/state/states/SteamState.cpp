...
void SteamState::update(MachineStateContext& context) {
    ...
    LOGF(DEBUG, "Steam: Active, Temp=%.1f°C, Tank=%s, Sensors=%s", 
...
std::unique_ptr<MachineState> SteamState::checkTransitions(MachineStateContext& context) {
    ...
    if (context.isEmergencyStop()) {
        context.logStateTransition(getStateId(), MachineStateIds::EMERGENCY_STOP, 
                                   "Emergency condition detected during steam");
    ...
    if (context.isHotWaterActive() && context.isSteamActive()) {
        context.logStateTransition(getStateId(), MachineStateIds::HOT_WATER, 
                                   "Hot water activated during steam mode");
    ...
    if (context.hasSensorError() || context.hasTemperatureError()) {
        context.logStateTransition(getStateId(), MachineStateIds::SENSOR_ERROR, 
                                   "Sensor error detected during steam");
