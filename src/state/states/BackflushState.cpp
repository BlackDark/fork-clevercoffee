...
void BackflushState::update(MachineStateContext& context) {
    ...
    LOGF(DEBUG, "Backflush: Active, Temp=%.1f°C, Tank=%s, Sensors=%s", context.getCurrentTemperature(), context.isWaterTankFull() ? "OK" : "EMPTY", context.hasSensorError() ? "ERROR" : "OK");
}
...
std::unique_ptr<MachineState> BackflushState::checkTransitions(MachineStateContext& context) {
    ...
    if (context.isEmergencyStop()) {
        context.logStateTransition(getStateId(), MachineStateIds::EMERGENCY_STOP, "Emergency condition detected during backflush");
        return std::make_unique<EmergencyStopState>();
    }

    // Check for water tank empty
    if (!context.isWaterTankFull()) {
        context.logStateTransition(getStateId(), MachineStateIds::WATER_TANK_EMPTY, "Water tank empty during backflush");
        return std::make_unique<WaterTankEmptyState>();
    }

    // Check for sensor errors
    if (context.hasSensorError() || context.hasTemperatureError()) {
        context.logStateTransition(getStateId(), MachineStateIds::SENSOR_ERROR, "Sensor error detected during backflush");
    ...
    if (!context.isBackflushActive()) {
        context.logStateTransition(getStateId(), MachineStateIds::PID_NORMAL, "Backflush cleaning completed");
