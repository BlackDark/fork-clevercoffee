...
void ManualFlushState::update(MachineStateContext& context) {
    ...
    LOGF(DEBUG, "Manual Flush: Active, Tank=%s, Sensors=%s", context.isWaterTankFull() ? "OK" : "EMPTY", context.hasSensorError() ? "ERROR" : "OK");
}
...
std::unique_ptr<MachineState> ManualFlushState::checkTransitions(MachineStateContext& context) {
    ...
    if (context.isEmergencyStop()) {
        context.logStateTransition(getStateId(), MachineStateIds::EMERGENCY_STOP, "Emergency condition detected during manual flush");
        return std::make_unique<EmergencyStopState>();
    }

    // Check for water tank empty
    if (!context.isWaterTankFull()) {
        context.logStateTransition(getStateId(), MachineStateIds::WATER_TANK_EMPTY, "Water tank empty during manual flush");
        return std::make_unique<WaterTankEmptyState>();
    }

    // Check for sensor errors
    if (context.hasSensorError() || context.hasTemperatureError()) {
        context.logStateTransition(getStateId(), MachineStateIds::SENSOR_ERROR, "Sensor error detected during manual flush");
    ...
    if (!context.isManualFlushActive()) {
        context.logStateTransition(getStateId(), MachineStateIds::PID_NORMAL, "Manual flush completed");
