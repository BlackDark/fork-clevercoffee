...
bool PID::Compute() {
    ...
    if (timeChange >= SampleTime) {
        ...
        // Integral Part
        // added: don't let I part sum grow if output is already at max (from e.g. P alone), c.f.
        ...
        //(the controller uses negative of derived input instead of derived error, since it's equal when assuming
        // setpoint is constant - solves derivative kick)
        double dInput = 0;
        if (pOnE) {
        ...
        // Apply output limits to I sum (worst case anti-windup, see
...
double PID::GetDeltaInput() {
    return lastFilteredDifferential;
}
double PID::GetInputError() {
    return lastError;
}
double PID::GetLastPPart() {
    return lastPPart;
}
double PID::GetLastIPart() {
    return integrator;
}
double PID::GetLastDPart() {
    return lastDPart;
}
